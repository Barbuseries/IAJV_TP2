	...
	LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		switch (wParam)
		{
		case ID_LEADERCONTROL_EDITGAMESETTINGS:
		{
			showMenu = true;
			g_GameWorld->TogglePause();
		}
		break;
		...
		if (menuOn) {
			g_GameMenu->HandleInput(wParam, hwnd);
		}
		else {
...
//make sure the window creation has gone OK
	...
	while (!bDone)
	{
		...
		if (showMenu) {
			g_GameMenu = new GameMenu(hWnd, winclass.hInstance);
			showMenu = false;
			menuOn = true;
		}
